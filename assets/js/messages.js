$(function(){
	$(document).on('click', '#messagePopup', function(){
		var getMessages = 1;
		$.post('http://localhost/BSbehance/core/ajax/messages.php', {showMessage:getMessages}, function(data){
	...
	$(document).on('click', '.people-message', function(){
		...
		$.post('http://localhost/BSbehance/core/ajax/messages.php', {showChatPopup:get_id}, function(data){
			$('.popupTweet').html(data);
			if(autoscroll){
				scrollDown();
			}
		...
		getMessages = function(){
			$.post('http://localhost/BSbehance/core/ajax/messages.php', {showChatMessage:get_id}, function(data){
		...
		$(document).on('click', '.deleteMsg', function(){
			...
			$(document).on('click', '.delete', function(){
				$.post('http://localhost/BSbehance/core/ajax/messages.php', {deleteMsg:messageID}, function(data){
					$('.message-del-inner').height('0px');
					getMessages();
				})
			});
